...
int main( int argc, char** argv )
{
    ...
    marker_frame1 = RosVizTools::newFrame(0.1, 2.0, pose, ns, 0, frame_id, lifetime);
    pose.position.x = -5.0;
    pose.position.y = 2.0;
    pose.position.z = -3.0;
    pose.orientation = tf2::toMsg(tf2::Quaternion(30 * M_PI / 180, 30 * M_PI / 180, 30 * M_PI / 180));
    marker_frame2 = RosVizTools::newFrame(0.1, 1.0, pose, ns, 1, frame_id, lifetime);
    ...
    ns = "cube_list";
    ++lifetime;
    visualization_msgs::Marker marker_cubelist =
        RosVizTools::newCubeList(0.5, ns, 0, ros_viz_tools::WHITE, frame_id, lifetime);
    for (int i = 0; i < 10; ++i) {
    ...
    visualization_msgs::Marker marker_linestrip =
        RosVizTools::newLineStrip(0.3, ns, 0, ros_viz_tools::LIGHT_BLUE, frame_id, lifetime);
    for (int i = 0; i < 10; ++i) {
        geometry_msgs::Point p;
        p.x = i;
        p.y = pow(p.x, 2.0);
    ...
    ns = "sphere_list";
    ++lifetime;
    visualization_msgs::Marker marker_spherelist =
        RosVizTools::newSphereList(1.0, ns, 0, ros_viz_tools::LIME_GREEN, frame_id, lifetime);
    for (int i = 0; i < 10; ++i) {
        geometry_msgs::Point p;
        p.x = i;
        p.y = pow(p.x, 2.0);
    ...
    visualization_msgs::Marker marker_cylinder =
        RosVizTools::newCylinder(scale, pose , ns, 0, ros_viz_tools::WHITE, frame_id, lifetime);
    ...
    visualization_msgs::Marker marker_cube =
        RosVizTools::newCube(1.0, pose , ns, 0, ros_viz_tools::WHITE, frame_id, lifetime);
    ...
    visualization_msgs::Marker marker_sphere =
        RosVizTools::newSphere(0.5, pose , ns, 0, ros_viz_tools::RED, frame_id, lifetime);
    ...
    visualization_msgs::Marker marker_arrow =
        RosVizTools::newArrow(scale, pose , ns, 0, ros_viz_tools::WHITE, frame_id, lifetime);
    ...
    ns = "colormap";
    lifetime = 0.0;
    visualization_msgs::Marker marker_colormap1 =
        RosVizTools::newLineStrip(0.2, ns, 0, ros_viz_tools::LIGHT_BLUE, frame_id, lifetime);
    visualization_msgs::Marker marker_colormap2 =
        RosVizTools::newSphereList(0.2, ns, 1, ros_viz_tools::LIGHT_BLUE, frame_id, lifetime);
    for (int i = 0; i < 20; ++i) {
        geometry_msgs::Point p;
        p.x = i * 0.5;
        p.y = sin(p.x);
